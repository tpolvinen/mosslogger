...
/* defines */
#define BYTES2READ (4)
#define HIH6030addr (0x27) 	/* slave I2c address */
...
int main(void) {
	...
	while (1) {
		/* kick the sensor */
		if (qm_i2c_master_write(QM_I2C_0, HIH6030addr, dummydata,
				sizeof(dummydata), true, &I2Cstatus)) {
			return (errno); 	/* error code (errn) */
		}
		clk_sys_udelay(50000);  /* 60 ms delay for measurement (~36.65 ms)*/
		/* get the data */
		rescode=qm_i2c_master_read(QM_I2C_0, HIH6030addr, HIHdata, BYTES2READ, true,
				&I2Cstatus);
		if(rescode) {
			QM_PRINTF("Error: Could not read HIH6030 (errno=%d)\n",rescode);
			return (1);
		}
